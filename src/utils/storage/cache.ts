...
const { DEFAULT_CACHE_PREFIX, DEFAULT_CACHE_TIME, DEFAULT_CACHE_CIPHER, ENABLE_ENCRYPTION } =
  config;
...
export function createStorage(options: CreateStorageOptions) {
  ...
  class Storage {
    ...
    get(key: string) {
      ...
      try {
        const decryptValue = this.hasEncrypt
          ? this.encryption.decryptByAes(storageValue)
          : storageValue;
